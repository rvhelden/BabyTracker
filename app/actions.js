...
export async function addMilkAction(babyId, _prevState, formData) {
  ...
  const duration_minutes = parseInt(formData.get('duration_minutes')?.toString() || '0', 10) || null;
  const notes = formData.get('notes')?.toString();
  ...
  dal.addMilk(babyId, user.id, { volume_ml, fed_at, notes, started_at, ended_at, duration_minutes });
  revalidatePath(`/baby/${babyId}`);
  return { success: true };
}
...
export async function updateMilkAction(babyId, entryId, _prevState, formData) {
  ...
  const updated = dal.updateMilk(babyId, entryId, { volume_ml, fed_at, notes, started_at, ended_at, duration_minutes });
  if (!updated) return { error: 'Entry not found' };
