...
try {
  db.exec("ALTER TABLE medication_entries ADD COLUMN max_interval_minutes INTEGER");
} catch {
  // Column already exists
}
try {
  db.exec("ALTER TABLE predefined_medications ADD COLUMN max_interval_minutes INTEGER");
} catch {
  // Column already exists
}
