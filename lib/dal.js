...
export function createBaby(name, birthDate, gender, ownerId) {
  ...
  ).run(name, birthDate, gender || null);
...
export function updateBaby(babyId, userId, { name, birth_date, gender }) {
  const access = prepare(
    'SELECT role FROM user_babies WHERE user_id = ? AND baby_id = ?'
  ).get(userId, babyId);
  if (!access) return null;
  ...
  ).run(name || null, birth_date || null, gender !== undefined ? gender || null : null, babyId);
