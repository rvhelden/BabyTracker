...
export function addMedicationEntry(
  babyId,
  userId,
  { medication_name, dosage, interval_minutes, max_interval_minutes, given_at, notes },
) {
  const result = prepare(
    "INSERT INTO medication_entries (baby_id, medication_name, dosage, interval_minutes, max_interval_minutes, given_at, notes, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
  ).run(
    babyId,
    medication_name,
    dosage || null,
...
export function updateMedicationEntry(
  ...
  prepare(
    "UPDATE medication_entries SET medication_name = COALESCE(?, medication_name), dosage = COALESCE(?, dosage), interval_minutes = ?, max_interval_minutes = ?, given_at = COALESCE(?, given_at), notes = COALESCE(?, notes) WHERE id = ?",
  ).run(
    medication_name || null,
    dosage !== undefined ? dosage : null,
    interval_minutes !== undefined ? (interval_minutes ?? null) : entry.interval_minutes,
    max_interval_minutes !== undefined
      ? (max_interval_minutes ?? null)
...
export function getLastMedicationEntryByName(babyId, medicationName) {
  return prepare(
    "SELECT * FROM medication_entries WHERE baby_id = ? AND LOWER(TRIM(medication_name)) = LOWER(TRIM(?)) ORDER BY given_at DESC LIMIT 1",
  ).get(babyId, medicationName);
}
...
export function addPredefinedMedication(
  babyId,
  userId,
  { medication_name, dosage, interval_minutes, max_interval_minutes },
) {
  const result = prepare(
    "INSERT INTO predefined_medications (baby_id, medication_name, dosage, interval_minutes, max_interval_minutes, created_by) VALUES (?, ?, ?, ?, ?, ?)",
  ).run(
    babyId,
    medication_name,
    dosage || null,
...
export function updatePredefinedMedication(
  ...
  prepare(
    "UPDATE predefined_medications SET medication_name = COALESCE(?, medication_name), dosage = ?, interval_minutes = ?, max_interval_minutes = ? WHERE id = ?",
  ).run(
    medication_name || null,
    dosage || null,
    interval_minutes ?? null,
