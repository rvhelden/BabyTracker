...
export function addMilk(babyId, userId, { volume_ml, fed_at, notes }) {
  const result = prepare(
    'INSERT INTO milk_entries (baby_id, volume_ml, fed_at, notes, created_by) VALUES (?, ?, ?, ?, ?)'
  ).run(babyId, volume_ml, fed_at, notes || null, userId);

  return prepare('SELECT * FROM milk_entries WHERE id = ?').get(result.lastInsertRowid);
}

export function updateMilk(babyId, entryId, { volume_ml, fed_at, notes }) {
  const entry = prepare(
  ...
  prepare(
    'UPDATE milk_entries SET volume_ml = COALESCE(?, volume_ml), fed_at = COALESCE(?, fed_at), notes = COALESCE(?, notes) WHERE id = ?'
  ).run(volume_ml || null, fed_at || null, notes !== undefined ? notes : null, entryId);
