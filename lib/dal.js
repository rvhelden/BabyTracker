...
export function updateWeight(babyId, entryId, { weight_grams, measured_at, notes }) {
  const entry = prepare("SELECT * FROM weight_entries WHERE id = ? AND baby_id = ?").get(
    entryId,
    babyId,
  );
...
export function hasWeightEntry(babyId, measured_at, weight_grams) {
  return prepare(
    "SELECT id FROM weight_entries WHERE baby_id = ? AND measured_at = ? AND weight_grams = ?",
