...
function resolveTimeZone() {
  const temporalTimeZone = Temporal.Now?.timeZoneId?.();
  if (typeof temporalTimeZone === "string" && temporalTimeZone) {
    return temporalTimeZone;
  }

  const intlTimeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
  if (typeof intlTimeZone === "string" && intlTimeZone) {
    return intlTimeZone;
  }
...
export function formatWeekdayShort(dateTime, locale) {
  if (!dateTime) {
    return "";
  }
  if (dateTime instanceof Temporal.PlainDate) {
    ...
    return zoned.toLocaleString(locale || undefined, { weekday: "short" });
  }
  if (dateTime instanceof Temporal.PlainDateTime) {
    return dateTime
      .toZonedDateTime(timeZone)
      .toLocaleString(locale || undefined, { weekday: "short" });
  }
  if (dateTime instanceof Temporal.ZonedDateTime) {
    return dateTime.toLocaleString(locale || undefined, { weekday: "short" });
  }
  return String(dateTime);
}

export function formatLocalDateTime(dateTime, locale) {
  if (!dateTime) {
    return "";
  }
  if (dateTime instanceof Temporal.PlainDateTime) {
    return dateTime
      .toZonedDateTime(timeZone)
      .toLocaleString(locale || undefined, { dateStyle: "medium", timeStyle: "short" });
  }
  if (dateTime instanceof Temporal.ZonedDateTime) {
    return dateTime.toLocaleString(locale || undefined, {
      dateStyle: "medium",
      timeStyle: "short",
    });
  }
...
export function formatLocalTime(dateTime, locale) {
  if (!dateTime) {
    return "";
  }
  if (dateTime instanceof Temporal.PlainDateTime) {
    return dateTime
      .toZonedDateTime(timeZone)
      .toLocaleString(locale || undefined, { timeStyle: "short" });
  }
  if (dateTime instanceof Temporal.ZonedDateTime) {
    return dateTime.toLocaleString(locale || undefined, { timeStyle: "short" });
  }
  return String(dateTime);
}
