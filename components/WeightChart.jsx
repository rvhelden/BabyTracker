...
function CustomTooltip({ active, payload }) {
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '0.6rem 1rem', boxShadow: 'var(--shadow)', fontSize: '0.88rem' }}>
      ...
      <p style={{ color: 'var(--primary-dark)' }}>{d.weight_grams} g ({(d.weight_grams / 1000).toFixed(3)} kg)</p>
    ...
    <div style={{ width: '100%', height: 300 }}>
      <ResponsiveContainer>
        <LineChart data={data} margin={{ top: 10, right: 20, left: 0, bottom: 5 }}>
          ...
          <XAxis dataKey="day" tickFormatter={formatDay} tick={{ fontSize: 12, fill: 'var(--text-muted)' }}
            label={{ value: 'Age', position: 'insideBottomRight', offset: -5, fontSize: 12, fill: 'var(--text-muted)' }} />
          <YAxis domain={[minKg, maxKg]} tickFormatter={v => `${v.toFixed(1)} kg`}
            tick={{ fontSize: 12, fill: 'var(--text-muted)' }} width={65} />
          <Tooltip content={<CustomTooltip />} />
          <Line type="monotone" dataKey="kg" stroke="var(--primary)" strokeWidth={2.5}
            dot={{ fill: 'var(--primary)', r: 4, strokeWidth: 0 }}
            activeDot={{ r: 6, fill: 'var(--primary-dark)' }} />
          {data[0]?.day === 0 && (
            <ReferenceLine x={0} stroke="var(--accent)" strokeDasharray="4 2"
