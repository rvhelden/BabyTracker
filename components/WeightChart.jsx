...
function CustomTooltip({ active, payload }) {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #dde3ed', borderRadius: 8, padding: '0.6rem 1rem', boxShadow: '0 2px 12px rgba(0,0,0,0.1)', fontSize: '0.88rem' }}>
    ...
    <div style={{ width: '100%', height: 300 }}>
      <ResponsiveContainer>
        <LineChart data={data} margin={{ top: 10, right: 20, left: 0, bottom: 5 }}>
          ...
          <XAxis dataKey="day" tickFormatter={formatDay} tick={{ fontSize: 12, fill: '#718096' }}
          ...
          <YAxis domain={[minKg, maxKg]} tickFormatter={v => `${v.toFixed(1)} kg`}
            tick={{ fontSize: 12, fill: '#718096' }} width={65} />
          <Tooltip content={<CustomTooltip />} />
          <Line type="monotone" dataKey="kg" stroke="#6c8ebf" strokeWidth={2.5}
            dot={{ fill: '#6c8ebf', r: 4, strokeWidth: 0 }}
            activeDot={{ r: 6, fill: '#4a6fa5' }} />
          {data[0]?.day === 0 && (
            <ReferenceLine x={0} stroke="#f5a623" strokeDasharray="4 2"
              label={{ value: 'Birth', fontSize: 11, fill: '#f5a623' }} />
          )}
        </LineChart>
      </ResponsiveContainer>
    </div>
