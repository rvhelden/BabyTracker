...
router.post('/', (req, res) => {
  ...
  const baby = prepare('SELECT * FROM babies WHERE id = ?').get(result.lastInsertRowid);
  res.status(201).json(baby);
});

// GET /api/babies/:id
router.get('/:id', (req, res) => {
  const access = prepare('SELECT role FROM user_babies WHERE user_id = ? AND baby_id = ?').get(req.user.id, req.params.id);
...
router.delete('/:id', (req, res) => {
  ...
  prepare('DELETE FROM babies WHERE id = ?').run(req.params.id);
...
router.delete('/:id/leave', (req, res) => {
  ...
  prepare('DELETE FROM user_babies WHERE user_id = ? AND baby_id = ?').run(req.user.id, req.params.id);
  res.json({ success: true });
});
